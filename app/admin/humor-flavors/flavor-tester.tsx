...
async function callGenerateCaptions(token: string, imageId: string, flavor: FlavorOption | null) {
  const payloadCandidates: Record<string, unknown>[] = flavor
    ? [
...
export function FlavorTester({ flavors, images, defaultFlavorId }: TesterProps) {
  ...
  const runTestSet = async () => {
    ...
    for (const imageId of selectedImageIds) {
      ...
      setStatusMessage(
        activeFlavor
          ? `Generating captions for image ${imageId.slice(0, 8)} with flavor "${activeFlavor.name}"...`
          : `Generating captions for image ${imageId.slice(0, 8)} using Humor Flavor Mix fallback...`
      )
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <label className="grid gap-1 text-sm">
          ...
          >
            <option value="">Use Humor Flavor Mix (no explicit flavor)</option>
            {flavors.map((flavor) => (
